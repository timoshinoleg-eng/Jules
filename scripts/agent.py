...
AI_PROTOCOL = PROVIDER_CONFIG["protocol"]
...
def calculate_backoff(attempt):
    """Вычисляет задержку между повторами по экспоненциальной схеме."""
...
def request_with_retry(method, url, headers=None, json_payload=None, expected_statuses=None, timeout=REQUEST_TIMEOUT):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if should_retry(response.status_code) and attempt < MAX_RETRIES - 1:
                delay = calculate_backoff(attempt)
                log(
                    f"Повтор {attempt + 1}/{MAX_RETRIES - 1} для {method} {url}: "
                    f"HTTP {response.status_code}, ожидание {delay:.1f} сек"
                )
                time.sleep(delay)
                continue
...
def github_request(method, url, json_payload=None, expected_statuses=None):
    """Выполняет запрос к GitHub API с повторными попытками."""
...
def build_ai_request(prompt):
    """Строит заголовки и payload для выбранного AI-провайдера."""
    ...
    raise ValueError(f"Неподдерживаемый протокол AI: {AI_PROTOCOL}")


def extract_ai_content(response_data):
    """Извлекает текст ответа из структуры выбранного AI-провайдера."""
    if AI_PROTOCOL == "anthropic":
...
def call_ai(prompt):
    ...
    log(f"Отправка запроса в AI через {API_TYPE} ({MODEL})...")
