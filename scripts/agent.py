...
def request_with_retry(method, url, headers=None, json_payload=None, timeout=REQUEST_TIMEOUT, expected_statuses=None):
    """Выполняет HTTP-запрос с экспоненциальной задержкой между повторами."""
    last_error = None
    last_response = None
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if should_retry(response=response) and attempt < MAX_RETRIES:
                delay = BACKOFF_FACTOR ** (attempt - 1)
                log(f"HTTP {response.status_code} для {method} {url}. Повтор через {delay:.1f} сек.")
                time.sleep(delay)
                continue
            ...
            last_error = error
            if attempt >= MAX_RETRIES or not should_retry(error=error):
                raise
            delay = BACKOFF_FACTOR ** (attempt - 1)
            log(f"Ошибка запроса {method} {url}: {error}. Повтор через {delay:.1f} сек.")
            time.sleep(delay)
...
def get_ci_logs():
    ...
    response = request_with_retry("GET", jobs_url, headers=HEADERS_GH, expected_statuses={200, 404})
...
def build_ai_request(prompt):
    """Строит запрос к выбранному AI-провайдеру."""
    ...
    raise ValueError(f"Неподдерживаемый формат запроса AI: {request_format}")


def extract_ai_content(data):
    """Извлекает текст ответа от выбранного AI-провайдера."""
    request_format = PROVIDER_CONFIG["request_format"]
    ...
    raise ValueError(f"Неподдерживаемый формат ответа AI: {request_format}")
...
def get_default_branch_sha():
    ...
    for branch in ["main", "master"]:
        ...
        response = request_with_retry("GET", url, headers=HEADERS_GH, expected_statuses={200, 404})
        if response.status_code == 200:
...
def get_file_sha(file_path, branch_name):
    """Возвращает SHA файла в указанной ветке, если файл существует."""
    get_url = f"{GITHUB_API}/repos/{REPO_FULL_NAME}/contents/{file_path}?ref={branch_name}"
    response = request_with_retry("GET", get_url, headers=HEADERS_GH, expected_statuses={200, 404})
    if response.status_code == 200:
        return response.json().get("sha")
    return None


def create_or_update_file(file_path, content, branch_name, action):
    ...
    response = request_with_retry("PUT", put_url, headers=HEADERS_GH, json_payload=payload)
    response.raise_for_status()
    log(f"{'Обновлён' if sha else 'Создан'} файл: {file_path}")


def delete_file(file_path, branch_name):
    """Удаляет файл из репозитория, если он существует."""
    sha = get_file_sha(file_path, branch_name)
    if not sha:
    ...
    response = request_with_retry("DELETE", delete_url, headers=HEADERS_GH, json_payload=payload)
    response.raise_for_status()
...
def create_branch_and_pr(changes, analysis):
    ...
    for change in changes:
        ...
        create_or_update_file(file_path, content, branch_name, action)
