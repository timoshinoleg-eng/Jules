...
MODEL = os.environ.get("MODEL", PROVIDER_CONFIG["default_model"]).strip() or PROVIDER_CONFIG["default_model"]
...
def is_retryable_response(response):
    """Проверяем, нужно ли повторить запрос по коду ответа."""
    return response.status_code in RETRYABLE_STATUS_CODES


def request_with_retry(method, url, *, headers=None, json_data=None, timeout=REQUEST_TIMEOUT, acceptable_statuses=None):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if response.status_code in acceptable_statuses or not is_retryable_response(response):
                return response
            ...
            last_exception = exc
            log(
                f"Ошибка сети при запросе {method} {url}: {exc}. "
                f"Попытка {attempt}/{MAX_RETRIES}"
            )
    ...
    raise RuntimeError(f"Не удалось выполнить запрос {method} {url}: {last_exception}")


def github_request(method, url, *, json_data=None, acceptable_statuses=None):
    """Обёртка для запросов к GitHub API."""
...
def build_anthropic_payload(prompt):
    """Формируем payload для Anthropic-совместимого API."""
    headers = {
        "x-api-key": API_KEY,
...
def build_openai_payload(prompt):
    """Формируем payload для OpenAI-compatible API."""
    headers = {
...
def parse_anthropic_response(data):
    """Извлекаем текст ответа из Anthropic-совместимого API."""
    content_blocks = data.get("content", [])
    if not content_blocks:
        raise ValueError("Пустой ответ от Anthropic API")
...
def parse_openai_response(data):
    """Извлекаем текст ответа из OpenAI-compatible API."""
    choices = data.get("choices", [])
    if not choices:
        raise ValueError("Пустой ответ от OpenAI-compatible API")
...
def get_file_content(path):
    ...
    resp = github_request("GET", url, acceptable_statuses={404})
...
def get_ci_logs():
    ...
    jresp = github_request("GET", jobs_url, acceptable_statuses={404})
...
def call_ai(prompt):
    ...
    builder_name = PROVIDER_CONFIG["request_builder"]
    parser_name = PROVIDER_CONFIG["response_parser"]
    headers, payload = globals()[builder_name](prompt)

    log(f"Отправка запроса в AI provider '{API_TYPE}' ({MODEL})...")
    ...
    resp.raise_for_status()
    data = resp.json()
    content = globals()[parser_name](data)
...
def create_branch_and_pr(changes, analysis):
    ...
    for change in changes:
        ...
        if action == "delete":
            ...
            gresp = github_request("GET", get_url, acceptable_statuses={404})
