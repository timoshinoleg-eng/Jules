...
def should_retry(method, status_code=None, error=None):
    """Определяет, нужно ли повторить HTTP-запрос."""
    if error is not None:
        return True
    if status_code in RETRYABLE_STATUS_CODES:
        return True
    if status_code == 409 and method.upper() in {"PUT", "POST", "DELETE"}:
        return True
    return False


def request_with_retry(method, url, headers=None, json_data=None, timeout=REQUEST_TIMEOUT, max_retries=HTTP_MAX_RETRIES):
    """Выполняет HTTP-запрос с экспоненциальной задержкой."""
    last_exception = None
    response = None
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if not should_retry(method, status_code=response.status_code):
                return response
            ...
            log(f"Повтор HTTP {method} {url} через {delay:.1f}с, статус {response.status_code}, попытка {attempt}/{max_retries}")
            time.sleep(delay)
        except requests.RequestException as exc:
            last_exception = exc
            if attempt == max_retries or not should_retry(method, error=exc):
                raise

            delay = HTTP_BACKOFF_BASE * (2 ** (attempt - 1))
            log(f"Сбой HTTP {method} {url}: {exc}. Повтор через {delay:.1f}с, попытка {attempt}/{max_retries}")
            time.sleep(delay)
...
class AIProvider:
    ...
    def call(self, prompt):
        raise NotImplementedError("Метод call должен быть реализован в провайдере")


class AnthropicProvider(AIProvider):
    """Клиент Anthropic-совместимого API."""
    ...
    def call(self, prompt):
        ...
        log(f"Отправка запроса в Claude через FreeModel ({self.model})...")
        resp = request_with_retry("POST", self.api_url, headers=headers, json_data=payload)
...
class OpenAICompatibleProvider(AIProvider):
    ...
    def call(self, prompt):
        ...
        log(f"Отправка запроса в FreeModel OpenAI-compatible ({self.model})...")
        resp = request_with_retry("POST", self.api_url, headers=headers, json_data=payload)
...
def get_ai_provider():
    """Создаёт клиента нужного AI-провайдера."""
    providers = {
    ...
    return provider_class(API_KEY, MODEL, API_URL)
...
def call_ai(prompt):
    """Отправляем запрос в выбранный AI API."""
